...
static inline size_t ggml_backend_sched_hash_get(ggml_backend_sched_t sched, struct ggml_tensor * tensor) {
    return ggml_hash_find_or_insert(&sched->hash_set, tensor);
}

#define hash_id(tensor) ggml_backend_sched_hash_get(sched, tensor)
...
static void ggml_backend_sched_split_graph(ggml_backend_sched_t sched, struct ggml_cgraph * graph) {
    ...
    struct ggml_cgraph * graph_copy = ggml_new_graph_custom2(sched->ctx, graph->n_nodes + sched->n_splits*GGML_SCHED_MAX_SPLIT_INPUTS*2, false, false);
